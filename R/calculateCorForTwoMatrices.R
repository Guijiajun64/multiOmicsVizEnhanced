#' Calculate Spearman Correlation Between Two Omics Matrices
#'
#' This function computes the gene-by-gene Spearman correlation between two 
#' omics matrices and applies FDR correction to identify significant 
#' correlations. It supports both matrix/data.frame inputs and 
#' `SummarizedExperiment` objects.
#'
#' @param matrix1 A matrix, data.frame, or `SummarizedExperiment` object. 
#'   Rows should be genes and columns samples.
#' @param matrix2 A matrix, data.frame, or `SummarizedExperiment` object 
#'   with matching samples.
#' @param fdr A numeric value indicating the FDR threshold.
#'
#' @return A correlation matrix where significant correlations are encoded 
#'   as +1 (positive), -1 (negative), and 0 (non-significant).
#'
...
#' \dontrun{
#' set.seed(1)
#' m1 <- matrix(rnorm(200), nrow = 20)
#' m2 <- matrix(rnorm(200), nrow = 20)
#' calculateCorForTwoMatrices(m1, m2, fdr = 0.05)
#' }
#' 
#' 
calculateCorForTwoMatrices <- function(matrix1,matrix2,fdr){
  ...
  if(inherits(matrix1, "SummarizedExperiment")){
    matrix1 <- assays(matrix1, 1)
  } else {
  ...
  if(inherits(matrix2, "SummarizedExperiment")){
    matrix2 <- assays(matrix2, 1)
  } else {
